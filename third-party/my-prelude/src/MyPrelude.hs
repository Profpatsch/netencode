...
-- | If the list is not empty, run the given function with a NonEmpty list, otherwise just return the mempty value for the result.
overNonEmpty :: (Applicative f, Monoid m) => (NonEmpty a -> f m) -> [a] -> f m
...
class (Foldable f) => Lengthy f where
  ...
  --
  -- ATTN: will only work for finite data structures under @maxBound :: Int@!!
  lengthNatural :: f a -> Natural
  lengthNatural xs =
    xs
      & Foldable.length
  ...
  -- | Get the element at the given index, if it exists (O(1) for vectors, O(n) for lists).
  atMay :: Natural -> f a -> Maybe a
  atMay = atMayViaList
  {-# INLINE atMay #-}

  -- | Get the last element, if it exists (O(1) for vectors, O(n) for lists).
  lastMay :: f a -> Maybe a
  lastMay = lastMayViaList
  {-# INLINE lastMay #-}

headMay :: (Lengthy f) => f a -> Maybe a
{-# INLINE headMay #-}
headMay = atMay 0

-- | If the data structure knows its length, we can use this to implement a (fast) safe at.
atMayLength :: (Lengthy f) => (Int -> f a -> a) -> Natural -> f a -> Maybe a
{-# INLINE atMayLength #-}
atMayLength lookupF idx f = do
...
-- | Every foldable can create a (slow) safe at, by converting to list.
atMayViaList :: (Foldable t) => Natural -> t a -> Maybe a
{-# INLINE atMayViaList #-}
atMayViaList idx xs = listAtMay idx $ toList xs

-- | A safe list index that does not need to know the length of the list.
listAtMay :: Natural -> [a] -> Maybe a
{-# INLINE listAtMay #-}
listAtMay idx xs = do
  let midx = integerToBounded @Int (idx & naturalToInteger)
  midx >>= \i -> f i xs
...
-- | Get the last element via (slow) traversal.
lastMayViaList :: (Foldable t) => t a -> Maybe a
{-# INLINE lastMayViaList #-}
lastMayViaList xs = xs & toList & nonEmpty <&> NonEmpty.last

-- | Get the last element via (fast) length lookup.
lastMayLength :: (Lengthy f) => f a -> Maybe a
{-# INLINE lastMayLength #-}
lastMayLength xs = atMay (lengthNatural xs - 1) xs
...
instance Lengthy Vector where
  atMay = atMayLength (flip (Vector.!))
  lastMay = lastMayLength
  lengthNatural xs =
    xs
      & Vector.length
      -- length can never be negative or something went really, really wrong
...
-- | If the boolean is true, return the value wrapped in `pure`, else return `empty`.
--
-- This is useful for functions like mapMaybe where the usual pattern is `if pred then Just a else Nothing`.
-- Now you can say @mapMaybe (\(k, v) -> v & guarded (k == "foo"))@.
guarded :: (Alternative m) => Bool -> a -> m a
guarded pred' a = if pred' then pure a else empty
...
-- | Like 'unwrapIOError', but for 'ErrorTree'. Will `throwIO` an uncatchable exception if 'Left'.
unwrapIOErrorTree :: Either ErrorTree a -> IO a
unwrapIOErrorTree e = e & first (newError . prettyErrorTree) & unwrapIOError
